...
func summarizeFetchModelsError(err error) string {
	if err == nil {
		return ""
	}
	message := strings.TrimSpace(err.Error())
	if idx := strings.Index(message, ", response:"); idx >= 0 {
		message = message[:idx]
	}
	if len(message) > 180 {
		message = message[:180] + "..."
...
func fetchModelsFallbackResponse(c *gin.Context, models []string, err error) bool {
	...
	c.JSON(http.StatusOK, gin.H{
		"success":  true,
		"fallback": true,
		"message":  fmt.Sprintf("上游获取模型失败（%s），已使用当前渠道已有模型", summarizeFetchModelsError(err)),
		"data":     models,
	})
	return true
}
...
func FetchUpstreamModels(c *gin.Context) {
	...
	if err != nil {
		if fetchModelsFallbackResponse(c, channel.GetModels(), err) {
			return
		}
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf("获取模型列表失败: %s", summarizeFetchModelsError(err)),
		})
		return
	}
...
func FetchModels(c *gin.Context) {
	...
	if baseURL == "" {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": fmt.Sprintf("base URL is required for channel type %d", req.Type),
		})
		return
	}
	...
	channel := &model.Channel{
		...
		Setting:        common.GetPointer(req.Setting),
		HeaderOverride: common.GetPointer(req.HeaderOverride),
	}
